...
COLLAR_DIR = "/home/juve/STAGE_BLUE_REFLET/bleu-reflet-collier-deploy (4)/data/usefull_necklace/newsize"
...
def process_image(image_path, model, output_dir, collar_path):
    ...
    print(f"✅ Collier '{os.path.basename(collar_path)}' placé sur {os.path.basename(image_path)}")

    img_name = os.path.splitext(os.path.basename(image_path))[0]
    col_name = os.path.splitext(os.path.basename(collar_path))[0]
    output_name = f"{img_name}__{col_name}.jpg"
    output_path = os.path.join(output_dir, output_name)
    os.makedirs(output_dir, exist_ok=True)
    cv2.imwrite(output_path, image)
    print(f"💾 Image sauvegardée : {output_path}")

def process_folder_multiple_collars(image_dir, collar_dir, model, output_dir):
    image_paths = glob(os.path.join(image_dir, "*.jpg")) + \
                  glob(os.path.join(image_dir, "*.jpeg")) + \
                  glob(os.path.join(image_dir, "*.png"))
    collar_paths = glob(os.path.join(collar_dir, "*.png"))

    for image_path in image_paths:
        for collar_path in collar_paths:
            process_image(image_path, model, output_dir, collar_path)

if __name__ == "__main__":
    process_folder_multiple_collars(IMAGE_DIR, COLLAR_DIR, model, OUTPUT_DIR)
